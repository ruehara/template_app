...
class AuthRepository implements IAuthRepository {
    ...
    try {
      final query = _database.select(_database.tafusuario).join([
        leftOuterJoin(
          ...
          _database.tafunidade.codunidade
              .equalsExp(_database.tafusuario.codunidade),
        ),
        leftOuterJoin(
          _database.tafperfil,
          _database.tafperfil.codperfil
              .equalsExp(_database.tafusuario.codperfil),
        ),
      ])..where(_database.tafusuario.desclogin.equals(username));

      final row = await query.getSingleOrNull();
      if (row == null) throw const AuthException('UsuÃ¡rio ou senha invÃ¡lidos');

      final usuario = row.readTable(_database.tafusuario);
      if (!BCrypt.checkpw(password, usuario.descsenha)) {
        throw const AuthException('UsuÃ¡rio ou senha invÃ¡lidos');
      }
      ...
      throw DatabaseException('Erro ao realizar login: $e');
    }
  }
  ...
  Future<AuthUserModel> getCurrentUser() async {
    try {
      ...
      if (userId == null) throw const AuthException('UsuÃ¡rio nÃ£o autenticado');
      ...
      final row = await query.getSingleOrNull();
      if (row == null) throw const DatabaseException('UsuÃ¡rio nÃ£o encontrado');

      return _buildAuthUser(row);
    } on AppException {
      rethrow;
    } catch (e) {
      throw DatabaseException('Erro ao buscar usuÃ¡rio: $e');
    }
  }
  ...
  Future<void> updateUser(String nome, String login, String? senha) async {
    try {
      ...
      throw DatabaseException('Erro ao atualizar usuÃ¡rio: $e');
    }
  }
  ...
  Future<void> register({
    ...
    try {
      final existingUser = await (_database.select(_database.tafusuario)
            ..where((u) => u.desclogin.equals(login)))
          .getSingleOrNull();
      if (existingUser != null) throw const AuthException('UsuÃ¡rio jÃ¡ existe');

      final unidade =
          await _database.select(_database.tafunidade).getSingleOrNull();
      if (unidade == null) {
        throw const DatabaseException(
          'Nenhuma unidade encontrada. Contate o administrador.',
        );
      }

      final perfil =
          await _database.select(_database.tafperfil).getSingleOrNull();
      if (perfil == null) {
        throw const DatabaseException(
          'Nenhum perfil encontrado. Contate o administrador.',
        );
      }

      final equipe =
          await _database.select(_database.tafequipe).getSingleOrNull();
      if (equipe == null) {
        throw const DatabaseException(
          'Nenhuma equipe encontrada. Contate o administrador.',
        );
      }

      await _database.into(_database.tafusuario).insert(
      ...
      throw DatabaseException('Erro ao registrar usuÃ¡rio: $e');
    }
  }
